...
let handler = {
  data: {
    //Setting 
    AnonymousUserPageList: ['ZhuanYe','ZhuanYeView','News','NewsView','Notify','ZiXun','ZiXunView','XiaoYouHui','XiaoYouMember','Activity','ActivityView','Cooperation','CooperationView','Schoolmate','SchoolmateView'],
    //INIT
    ...
    pageActionViewType: 'ListTemplate1',
    pageActionAddEdit:false,
    pageActionInit: false,
    pageActionInitType: 'ListTemplate1',
    ...
    ReadNumber: 0,
    viewData: {},
    uploadImageList: [],    //上传MobileEndNewsLeftImage时候所使用的临时变量
  ...
  开始去评论: function (e) {
    this.setData({
        PageBottonCommentDialog: true
    })
    this.打开页面底部的评论对话框();
  },

  打开页面底部的评论对话框: function (e) {
    let that = this;
    // 创建动画实例
  ...
  点击收藏: function (e) { 
    ...
    PostData['status'] = !this.data.IsFavorite; 
    PostData['id'] = id; 
    wx.request({
        ...
        success: function (res) {  
            if(res && res.data && res.data['status']=="OK") {
                let FavoriteNumber = that.data.IsFavorite ? Number(that.data.FavoriteNumber) - 1 : Number(that.data.FavoriteNumber) + 1
                that.setData({
  ...
  点击点赞: function (e) {
    ...
    PostData['action'] = action; 
    PostData['domain'] = domain; 
    PostData['status'] = !this.data.IsLike; 
    PostData['id'] = id; 
    wx.request({
        url: getApp().globalData.SYSTEM_URL + getApp().globalData.MainSummaryList,
        method: 'POST',
        ...
        success: function (res) {  
            if(res && res.data && res.data['status']=="OK") {
                let LikeNumber = that.data.IsLike ? Number(that.data.LikeNumber) - 1 : Number(that.data.LikeNumber) + 1
                that.setData({
  ...
  点击报名: function (e) {
    ...
    wx.request({
        ...
        success: function (res) {  
            if(res && res.data && res.data['status']=="OK") {
                if(action=="Enroll") {
                    that.data.viewData['MobileEndActivityMyEnrollStatus'] = 1
                    that.data.viewData['MobileEndActivityHaveEnrollNumber'] += 1                    
                }
                else {
                    that.data.viewData['MobileEndActivityMyEnrollStatus'] = 0
                    that.data.viewData['MobileEndActivityHaveEnrollNumber'] -= 1  
                }
                that.setData({
                    setData: that.data.viewData
                }) 
                wx.showToast({
                    title: res.data['msg'],
                    icon: 'success',
        ...
        if (data.无数据时提示 != "" && data.无数据时提示!= undefined && data.无数据时提示!= "undefined")   {
          ...
          if (data.init_default.MobileEndShowType == "无限表格")          {
            that.setData({
              ...
              Records: 记录列表,
              备注内容: data.备注内容,
              pageActionInitType: data.init_default.MobileEndShowType,
              pageActionInit: true,
              searchLoading: false,
            });
          }
  ...
  fetchInitPageList: function () {
    ...
    util.GetMainData(hostLastUrl + '?' + PageGetUrl, PostData, searchFieldName, searchFieldValue, pageid, callbackcount, function (data) {      
      ...
      that.data.pageActionInitType = data.init_default.MobileEndShowType;
      that.data.NewActionName = data.init_default.button_add;
      that.data.NewActionUrl  = "?action=add_default";
      that.data.actionValue = data.init_action.action;
      ...
      if(that.data.actionValue == "init_default") {
      ...
      else if(that.data.actionValue == "edit_default") {
        that.data.PageType          = "InterfaceAddEdit";
        that.data.PageAction        = "action="+that.data.actionValue+"&pageid=0&id="+data.init_action.id;
        that.data.pageIdEdit        = 0;
        that.data.pageRowId         = data.init_action.id;
        that.data.pageExternalId    = 0;        
        that.setData({
            pageActionInit: false,
            pageActionAddEdit: true,
            hiddenLoading:false,
        });
        that.AddAndEditPage(data.edit_default);
      }
  ...
  onLoadData: function (options,methodValue) {
    ...
    else if (that.data.PageID == "ZiXun")  {
        that.data.PageURL           = "/apps/apps_353.php";
        that.data.PageHeaderNAME    = "校友资讯";
        that.data.WechatCacheMark    = "ZiXun";
    }
    else if (that.data.PageID == "ZiXunView")  {
        that.data.PageURL           = "/apps/apps_353.php";
        that.data.PageHeaderNAME    = "校友资讯";
        that.data.WechatCacheMark    = "ZiXun";
    }
    else if (that.data.PageID == "Activity")  {
        that.data.PageURL           = "/apps/apps_362.php";
        that.data.PageHeaderNAME    = "活动";
        that.data.WechatCacheMark    = "Activity";
    }
    else if (that.data.PageID == "ActivityView")  {
        that.data.PageURL           = "/apps/apps_362.php";
        that.data.PageHeaderNAME    = "活动";
        that.data.WechatCacheMark    = "Activity";
    }
    else if (that.data.PageID == "Cooperation")  {
        that.data.PageURL           = "/apps/apps_363.php";
        that.data.PageHeaderNAME    = "校友互助";
        that.data.WechatCacheMark    = "Cooperation";
    }
    else if (that.data.PageID == "CooperationView")  {
        that.data.PageURL           = "/apps/apps_363.php";
        that.data.PageHeaderNAME    = "校友互助";
        that.data.WechatCacheMark    = "Cooperation";
    }
    else if (that.data.PageID == "Schoolmate")  {
        that.data.PageURL           = "/apps/apps_365.php";
        that.data.PageHeaderNAME    = "校友信息";
        that.data.WechatCacheMark    = "Schoolmate";
    }
    else if (that.data.PageID == "SchoolmateView")  {
        that.data.PageURL           = "/apps/apps_365.php";
        that.data.PageHeaderNAME    = "校友信息";
        that.data.WechatCacheMark    = "Schoolmate";
    }
  ...
  GetFormInforForView: function (ADD_OR_EDIT, INIT_DEFAULT) {
    ...
    wx.request({
      ...
      success: function (res) {  
        ...
        var pageActionViewType = INIT_DEFAULT['MobileEndShowType'];
        util.log("pageActionViewType: "+ pageActionViewType)
        if (pageActionViewType=='ListTemplate1')               {
            ...
            that.setData({
                pageActionViewType: pageActionViewType,
        ...
        else if (pageActionViewType=='NewsTemplate1' 
            || pageActionViewType=='ZiXun' 
            || pageActionViewType=='Activity' 
            || pageActionViewType=='Schoolmate' 
            || pageActionViewType=='NotificationTemplate1' 
            || pageActionViewType=='NotificationTemplate2')               {
          var setData = {};
          setData               = res.data['MobileEnd'];
          setData['id']         = that.data.pageRowId; 
          that.data.viewData    = setData
          that.setData({
            pageActionViewType: pageActionViewType,
            setData: setData,
            data: res.data['data'],
            ...
            hiddenLoading: true,//关闭加载中提示窗口
            SYSTEM_URL: getApp().globalData.SYSTEM_URL,
            LikeNumber: setData['MobileEndNewsLikeCounter'],
            FavoriteNumber: setData['MobileEndNewsFavoriteCounter'],
          });
          that.data.ReadNumber = setData['MobileEndNewsReadCounter']
          that.data.LikeNumber = setData['MobileEndNewsLikeCounter']
          that.data.FavoriteNumber = setData['MobileEndNewsFavoriteCounter']
        }
  ...
  bindRegionChange: function (e) {
    var that = this;
    //util.log("MobileEndNewsLeftImage上传成功:"); util.log(res.statusCode);
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data['status']=="ERROR")   {
          ...
          util.showModal("错误提示", res.data['msg']);
          return false;
        }
          ...
          util.showModal("错误提示", "可能出现代码错误,请检查接口");
          return false;
        }
  ...
  日期控件: function (e) {
    ...
    var ChooseValue = e.detail.value;
    var showFieldName = e.currentTarget.dataset.name;
    util.log(ChooseValue);
    //此变量赋值写法比较特殊,是利用变量指针,对动态变量进行赋值.
    var showFieldId = e.currentTarget.dataset.id;
    let updateData = {};
    updateData['liebiao[' + showFieldId + '].updateValue']            = ChooseValue;
    this.setData(updateData);
  },
  ...
  */
  绑定二级下拉选择框_列改变: function (e) {
    ...
    updateData['liebiao[' + showFieldId + '].微信小程序_用户选择索引'] = showFieldValue;
  ...
  绑定三级下拉选择框_列改变_存值: function (e) {
    ...
    switch (e.detail.column) {
      ...
      case 0:
        ...
        data[showFieldId]['微信小程序_左右下拉数组'] = range
        data[showFieldId]['微信小程序_用户选择索引'] = showFieldValue
        data[showFieldId]['微信小程序_用户选择名称']['省'] = that.data.ThreeSelect_one
        data[showFieldId]['微信小程序_用户选择名称']['市'] = that.data.ThreeSelect_two
        data[showFieldId]['微信小程序_用户选择名称']['区'] = that.data.ThreeSelect_three
        break;
      case 1:
        //e.detail.value的值是一个动态变量,不需要
    ...
    data[showFieldId]['微信小程序_用户选择索引'] = showFieldValue
    //data[showFieldId]['用户答案'] = that.data.ThreeSelect_one +"-"+ that.data.ThreeSelect_two +"-"+ that.data.ThreeSelect_three;
    data[showFieldId]['FieldCheckResult'] = "/images/yes01.png";
    data[showFieldId]['行政区代码'] = 行政区代码;
    this.setData({data:data});
    //util.log(that.data.ThreeSelect_one + that.data.ThreeSelect_two + that.data.ThreeSelect_three);
    //util.log(data[showFieldId]);
  },
  ...
  MobileEndNewsLeftImage放大: function (e) {
    ...
    wx.previewImage({
      current: allimagelist[id],     //当前MobileEndNewsLeftImage地址
      urls: allimagelist,               //所有要预览的MobileEndNewsLeftImage的地址集合 数组形式
      success: function (res) { },
      fail: function (res) { },
  ...
  上传多个MobileEndNewsLeftImage: function (e) {
    var that = this;
    //util.log(e);
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        //多选MobileEndNewsLeftImage进行直接赋值.不需要判断上传是否成功,直接给MobileEndNewsLeftImage进行赋值.
        //当前部分的赋值操作,只处理了微信小程序端的显示部分,并没有实质的处理上传操作.
        that.data.uploadImageList = res.tempFiles;
        //临时禁用FORM提交,以方便于MobileEndNewsLeftImage提交
        //启用FORM提交按钮．
        let updateData = {};
        updateData['isFormSubmitButtonDisabled'] = true;
        updateData['isFormSubmitButtonValue'] = '正在上传文件,请稍候...';
        that.setData(updateData);
        //util.log("最初使用的多个MobileEndNewsLeftImage上传:");
        //util.log(当前手动上传的MobileEndNewsLeftImage);
        for (var j = 0; j < that.data.uploadImageList.length; j++) {
          //实际上传文件
          that.上传多个MobileEndNewsLeftImage_上传操作(that.data.uploadImageList[j]['path'], e, j);
          //输出日志
  ...
  上传多个MobileEndNewsLeftImage_上传操作: function (filePath, e, j) {
    var that = this; 
    ...
    wx.uploadFile({
      ...
      success: function (res) {
        ...
        if (res.statusCode == 200) {
          ...
          updateData['liebiao[' + showFieldId + '].微信小程序_MobileEndNewsLeftImage列表'] = allimagelist;
        ...
        success: function (res) {
          ...
          if (ress['status'] == "OK") {
            ...
            if (prevPage!= undefined)           {
              ...
              else if(prevPage.data.indexmenu && prevPage.data.indexmenuGroup && prevPage.data.indexmenuGroup.length>0)  {
                //成功,返回到主页
                setTimeout(function () {
                  wx.navigateBack({
                    delta: 1
