...
Page({
  ...
  初始化页面()                        {
    ...
    wx.request({
      url: getApp().globalData.SYSTEM_URL+'/general/EDU/Interface/TDFORMICAMPUS/wechat.payment.campus_dayuhengshi_jiaofeiqita.php?action=search',
      method: 'POST',
      data: {
        DATA:DATA,
        stutype:"new",
  ...
  确认缴费操作: function (e) {
    ...
    var moneyamount = e.currentTarget.dataset.moneyamount;
    wx.showModal({
      title: triptitle,
      content: tripcontent+moneyamount+"?",
      success: function (res) {
        util.log(res);
        if (res.confirm) {
          that.确认缴费操作_实际执行(e);
  ...
  确认缴费操作_实际执行(e){
    ...
    表单所有字段值['JiaoFeiItemName'] = util.base64encode(e.currentTarget.dataset.moneyname);
    表单所有字段值['JiaoFeiItemAmount'] = e.currentTarget.dataset.moneyamount;
    表单所有字段值['JiaoFeiType'] = "CampusStore";
    ...
    let 学期本次缴费 = e.currentTarget.moneyamount;
